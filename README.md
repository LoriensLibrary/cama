...
**Reviewing for a role?** Start with **[EVIDENCE.md](EVIDENCE.md)** — a single matrix of every claim across the portfolio, paired with where to verify it and what the scope limit is. For the retrieval algorithm specifically (Phase-1 keyword routing, Phase-2.x embedding variants, blended scoring, counterweight injection, and the measured latency), see **[RETRIEVAL.md](RETRIEVAL.md)** — a drill-down companion to ARCHITECTURE.md. Then drop into the section below that matches what you care about.
