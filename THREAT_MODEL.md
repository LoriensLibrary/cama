...
| **13** | **Audit log tampering** by a compromised host | Attacker with shell access edits `api_audit_log` | The audit log is append-only at the API layer (no `UPDATE` or `DELETE` paths exposed). At the SQL layer, a compromised host can still mutate the file. The mitigation is operational: (a) ship audit rows to a write-only remote log sink if compliance requires (HIPAA-grade); (b) include a per-row hash chain (sha256(prev_hash + row_canonical_json)) so any later tampering is detectable on full replay. The chain is v1.1 work. | **design** + v1.1 |
