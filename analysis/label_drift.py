"""label_drift.py - First-pass register/drift labeler for CAMA."""
...
SHARP = [
    ...
    r"\bbro\b", r"\bsmh\b", r"\bnot okay\b", r"\bcorrect(ion|ing)?\b",
    r"\bur (regressing|drifting|missing|trippin)\b",
    r"\bu (cant|cannot|stopped|forgot)\b",
]
TASK = [
    r"\bcan you (help|write|review|build|fix|update|edit|check)\b",
    ...
    r"\bshould i\b", r"\blet'?s (build|make|write|do|review|update|finish|start)\b",
...
def umsg(ctx):
    ...
    m = re.search(r"Angela said:\s*(.+?)(?:\s*\||\s*$)", ctx, re.DOTALL)
...
def main():
    ...
    ap.add_argument("--out", default="cama_drift_labeled.csv")
    ...
    by_reg = dict(warm=[],sharp=[],task=[],mixed=[],unknown=[])
    by_plat = dict(claude=[],gpt=[])
    by_reg_plat = {}
    ...
    with open(args.out, "w", newline="", encoding="utf-8") as f:
        ...
        for row in rows:
            u = umsg(row["context"])
            ir = reg(u)
            st = feats(row["raw_text"])
            ds = drift(row["raw_text"], ir, st)
            pl = plat(row["context"], row["source_msg_id"])
            counts[ir] += 1
            by_reg[ir].append(len(ds))
            by_plat[pl].append(len(ds))
