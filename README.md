...
**Reviewing for a role?** Start with **[EVIDENCE.md](EVIDENCE.md)** — a single matrix of every claim across the portfolio, paired with where to verify it and what the scope limit is. For the retrieval algorithm specifically, see **[RETRIEVAL.md](RETRIEVAL.md)** — a drill-down companion to ARCHITECTURE.md. For the public HTTP API surface that lets CAMA be embedded in any AI application (not just Claude Desktop via MCP), see **[API.md](API.md)** + **[THREAT_MODEL.md](THREAT_MODEL.md)**. Then drop into the section below that matches what you care about.
